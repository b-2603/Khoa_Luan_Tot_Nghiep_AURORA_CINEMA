...
class AdminController
{
    ...
    {
        if (!isset($this->resources[$resource])) { jsonResponse(array('success' => false, 'message' => 'Resource không tồn tại.'), 404); }
        requireAdmin(); $input = requestJson(); $cfg = $this->resources[$resource];
    ...
    public function delete($resource)
    {
        ...
        if (!$id) { jsonResponse(array('success' => false, 'message' => 'Thiếu id.'), 400); }
        $this->execute("DELETE FROM {$this->resources[$resource]['table']} WHERE id = ?", 'i', array($id));
        jsonResponse(array('success' => true, 'message' => 'Đã xóa dữ liệu.'));
    }
    ...
    public function createTransaction() {
        requireAdmin(); $input = requestJson();
        ...
        if ($amount < 0 || !in_array($channel, array('pos','website','ota'), true)) { jsonResponse(array('success' => false, 'message' => 'Dữ liệu giao dịch không hợp lệ.'), 400); }
        $stmt = $this->db->prepare('INSERT INTO tms_transactions (transaction_code,customer_id,channel,amount,payment_method,status) VALUES (?,?,?,?,?,?)');
        $status = 'paid';
        $stmt->bind_param('sisdss', $code, $customer, $channel, $amount, $method, $status);
        if (!$stmt->execute()) { jsonResponse(array('success' => false, 'message' => $stmt->error), 500); }
    ...
    public function updateRefund() {
        ...
        if (!$id || !in_array($status, array('approved','rejected','completed'), true)) { jsonResponse(array('success' => false, 'message' => 'Dữ liệu hoàn tiền không hợp lệ.'), 400); }
        $this->execute("UPDATE tms_refunds SET status=?, processed_at=NOW() WHERE id=?", 'si', array($status, $id));
        if ($status === 'completed') { $this->db->query("UPDATE tms_transactions t JOIN tms_refunds r ON r.transaction_id=t.id SET t.status='refunded' WHERE r.id=" . $id); }
        jsonResponse(array('success' => true, 'message' => 'Đã cập nhật yêu cầu hoàn tiền.'));
    }
    
    public function seats() {
        $screen = isset($_GET['screen_id']) ? (int)$_GET['screen_id'] : 0;
        if (!$screen) { jsonResponse(array('success' => false, 'message' => 'Thiếu screen_id.'), 400); }
    ...
    public function report() {
        ...
        jsonResponse(array('success' => true, 'data' => array('from' => $from, 'to' => $to, 'summary' => $summaryRow, 'daily' => $daily)));
    }

    private function scalar($sql) { $result = $this->db->query($sql); $row = $result ? $result->fetch_row() : array(0); return isset($row[0]) ? (int)$row[0] : 0; }
